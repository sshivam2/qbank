...
function jwtIsExpired(token) {
  ...
  try {
    ...
    return !!obj.exp && Math.floor(Date.now() / 1000) >= obj.exp;
  } catch { return false; }
}
...
export async function middleware(request) {
  ...
  if (pathname === '/login.html') return NextResponse.next();
...
// Protect all routes except API, Next.js internals, static files, and login
export const config = {
  matcher: [
    '/((?!api|_next|_static|favicon.ico|.*\\.(?:png|jpg|jpeg|gif|webp|svg)|login.html).*)'
  ]
};
